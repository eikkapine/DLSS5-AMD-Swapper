...
This checkpoint publishes the native-output / low-resolution-neural path with the dev.6 compositor changes.
...
The previous dev.5 compositor intentionally discarded most low/mid-frequency neural reconstruction, which made the result look dominated by sharpening. Dev.6 restores a broader temporally matched neural delta while keeping the low-resolution neural branch.

No PresentMon-backed game/display frame-rate capture was recorded for this checkpoint, so this release publishes no FPS number for it.
