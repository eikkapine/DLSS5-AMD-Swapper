...
- `Validate(root)` produces `OptiScalerPackage` or throws with a single actionable sentence. Rules:
  ...
  - When `SHA256SUMS.txt` exists every listed file that exists is hashed; any mismatch is a hard failure naming the file. Listed files that are absent are reported as warnings, except the OptiScaler binary and pass 1 which stay required.
